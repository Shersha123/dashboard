...
function UserDashboard() {
  ...
  const AddtoCart = async (id) => {
    try {
      const user = await axios.post(`https://crm-backend-eight.vercel.app/addtocart/${id}`);
      get_cart_items();
    } catch (error) {
      console.log(error);
    }
  ...
  const get_cart_items = async () => {
    try {
      const cartItems = await axios.get(
        "https://crm-backend-eight.vercel.app/get_cart_items",
        {
          headers: {
            authorization: `${window.localStorage.getItem("token")}`,
          },
  ...
  const removeCart = async (id) => {
    try {
      ...
      const removeitem = await axios.delete(
        `https://crm-backend-eight.vercel.app/remove_cart_item/${id}`,
        {
          headers: {
            authorization: `${window.localStorage.getItem("token")}`,
          },
